...
#subfunction of remove_session used to remove user access
{
	...
	{
		iptables -t mangle -D iptmon_rx -o "${HS_INTERFACE}" -d "${1}" > /dev/null # delete rules since this user is effectively logged out
		iptables -t mangle -D iptmon_tx -i "${HS_INTERFACE}" -s "${1}" > /dev/null # same as above
		iptables -t nat -D PREROUTING -p all -s "${1}" -i "${HS_INTERFACE}" -j RETURN > /dev/null
		iptables -t filter -D FORWARD -p all -s "${1}" -i "${HS_INTERFACE}" -j ACCEPT > /dev/null  # Fixed: was ${USER_IP}
		iptables -t filter -D FORWARD -p all -d "${1}" -o "${HS_INTERFACE}" -j ACCEPT > /dev/null  # Fixed: was ${USER_IP}
	} 2>>error.log
}
...
function shutdown_infra {
    ...
    local local_ip_cache
	local_ip_cache=$(ip -4 addr show dev "${HS_INTERFACE}" | grep inet | awk '{print $2}' | cut -d '/' -f 1)
	###############
...
#subfunction of check_internet_sessions used to determine if user is 'over limits' (returns a boolean)
{
	...
	if [[ "$USER_LEVEL" == '0' ]]; then #if the user is an admin then skip all other checks and return 0 (i.e. not over limits)
		...
		{
		RESPONSE2=$(sqlite3 "${DATABASE_PATH}" "SELECT bytelimit_daily,bytelimit_weekly,bytelimit_monthly FROM data_lanes WHERE lane_id=${LANE_ID}") > /dev/null
		LANE_DAILY_LIMIT=$(($(echo "${RESPONSE2}" | cut -f 1 -d '|'))) > /dev/null
		LANE_WEEKLY_LIMIT=$(($(echo "${RESPONSE2}" | cut -f 2 -d '|'))) > /dev/null
		LANE_MONTHLY_LIMIT=$(($(echo "${RESPONSE2}" | cut -f 3 -d '|'))) > /dev/null
		} 2>> error.log
		if ((TOTAL_TX_DAY + TOTAL_RX_DAY > LANE_DAILY_LIMIT)); then
			return 1
		elif ((TOTAL_TX_WEEK + TOTAL_RX_WEEK > LANE_WEEKLY_LIMIT)); then
			return 1
		elif ((TOTAL_TX_MONTH + TOTAL_RX_MONTH > LANE_MONTHLY_LIMIT)); then
			return 1
		else
			return 0
		fi
...
#main function used to periodically update internet session data in the sqlite database and remove sessions based certain criteria.
{
	...
	do
		...
		{
			SESSION_TX=$(($(iptables -t mangle -L iptmon_tx -vxn | grep "${USER_IP}" | awk '{print $2}'))) > /dev/null
			SESSION_RX=$(($(iptables -t mangle -L iptmon_rx -vxn | grep "${USER_IP}" | awk '{print $2}'))) > /dev/null
		...
		if [[ $? -eq 0 && "$SESSION_ACCESS" == '0' ]]; then #rules exists but shouldn't
			{
				...
				iptables -t filter -D FORWARD -p all -s "${USER_IP}" -i "${HS_INTERFACE}" -j ACCEPT > /dev/null
				iptables -t filter -D FORWARD -p all -d "${USER_IP}" -o "${HS_INTERFACE}" -j ACCEPT > /dev/null
			} 2>> error.log
		fi
		iptables -t nat -C PREROUTING -s "${USER_IP}" -i "${HS_INTERFACE}" -j RETURN > /dev/null 2>> error.log
		if [[ $? -eq 1 && "$SESSION_ACCESS" == '1' ]]; then #rule doesn't exist but should
			{
				iptables -t nat -I PREROUTING 1 -p all -s "${USER_IP}" -i "${HS_INTERFACE}" -j RETURN > /dev/null
				iptables -t filter -I FORWARD 1 -p all -s "${USER_IP}" -i "${HS_INTERFACE}" -j ACCEPT > /dev/null #this rule will allow requests outside of the network for this user
				iptables -t filter -I FORWARD 1 -p all -d "${USER_IP}" -o "${HS_INTERFACE}" -j ACCEPT > /dev/null
			} 2>> error.log
		fi
